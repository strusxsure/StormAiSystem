...
async function generateWithOpenRouter(
    ...
    } else if (modelName === 'allenai/molmo-2-8b:free') {
...
export const generateWebsitePlan = async (userPrompt: string, modelName: string = 'gemini-3-pro-preview'): Promise<string> => {
    ...
    if (modelName === 'mimo-v2-flash' || modelName === 'allenai/molmo-2-8b:free') {
...
export const generateWebsiteCode = async (
  ...
  let systemInstruction = `
      ...
      **DOUBLE-CHECK ALL ICONS:** Before you output the code, review every icon component (e.g., the 'Zap' or 'User' components) and verify that it is listed in the \`i mport { ... } from 'lucide-react';\` statement. Every single icon must be imported.
    `;
