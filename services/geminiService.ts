...
function addLucideImports(code: string): string {
    ...
    allLucideIcons.forEach(icon => {
        // Match as a tag: <Icon ... /> or <Icon>
        const tagRegex = new RegExp(`<${icon}(\\s|\\/>|>)`);
        // Match as a property or variable (common in icon maps): icon={Icon} or component: Icon
        const propRegex = new RegExp(`[:={(]\\s*${icon}\\b`);

        if (tagRegex.test(code) || propRegex.test(code)) {
            usedIcons.add(icon);
        }
    });
...
export const generateWebsiteCode = async (
  ...
  let systemInstruction = `
      ...
      **CRITICAL SYNTAX RULES:**
      ...
      6. **MULTI-PAGE NAVIGATION (CRITICAL):**
         - If the user asks for multiple pages (e.g., "Home, Services, Contact"), you MUST build ALL of them in the single code block provided.
         - Manage the current page with a state variable: \`const [currentPage, setCurrentPage] = useState('home');\`
         - Use a standard navigation bar component that sets this state when links are clicked.
         - Render content conditionally: \`{currentPage === 'home' && <Home />} {currentPage === 'services' && <Services />}\` etc.
         - Do NOT omit any pages. Build the full, functional logic for every requested page.
         - For buttons or links that should navigate, always use \`onClick={() => setCurrentPage('pagename')}\`.
